...
fn tr(lang: &str, key: &str) -> &'static str {
    ...
    match key {
        ...
        "overlap_warning" => if en { "Overlap between note {a} (ends at beat {aend}) and note {b} (starts at beat {bstart})" } else { "Sobreposição entre nota {a} (fim={aend}) e nota {b} (início={bstart})" },
...
fn save_song(out_dir: String, song: Song, lang: String) -> Result<SaveResult, String> {
    ...
    Ok(SaveResult {
        txt_path: txt_path.to_string_lossy().to_string(),
        // O aviso é montado AQUI, não no rust-core: aquele crate não conhece
        // idioma, e a frase fixa em português chegava assim a quem usa o app
        // em inglês.
        warnings: song
            .find_overlaps()
            .into_iter()
            .map(|o| {
                tr(&lang, "overlap_warning")
                    .replace("{a}", &o.first_index.to_string())
                    .replace("{aend}", &o.first_end_beat.to_string())
                    .replace("{b}", &o.second_index.to_string())
                    .replace("{bstart}", &o.second_start_beat.to_string())
            })
            .collect(),
    })
}

...
- **A synced-lyrics line start could strand the first word of a line seconds ahead of the rest of it.** `seed_line_anchors` guarded only one direction: it refused a line start landing AFTER the next measured anchor, but happily accepted one landing far BEFORE. Real case (2026-09-06, "Ministry - Effigy (I'm Not An)"): the `.lrc` opens "But them burning pictures of my head to toe, hey!" at 40.56s, while WhisperX measured the rest of that same line from 45.24s. The seed went onto "But" and the two sources ended up spliced into one verse — the first word 4.7 seconds early and alone, then a hole until the rest of the phrase. Worse, the previous line was squeezed into 1.58 seconds for seven words, wedged between the two seeds. A line start is now refused when it sits more than 3 seconds before the first MEASURED word of its OWN line: within a single line, a measurement taken from this recording beats a timestamp from a file. The word falls back to interpolation, which spreads it between its neighbours — no hole, no squeezed verse. Deliberately limited to within one line, because a long gap BETWEEN lines is normal (an instrumental break) and judging against the next anchor anywhere would reject good seeds exactly there. Skipped in override mode, alongside the existing monotonicity checks, so the low-recognition path that fixed "Cause & Effect - Inside Out" cannot regress.

...
- **The setup now installs Deno, the JavaScript runtime yt-dlp needs for YouTube.** Downloads had started printing "No supported JavaScript runtime could be found ... YouTube extraction without a JS runtime has been deprecated, and some formats may be missing". YouTube hands out JavaScript challenges and yt-dlp solves them by running a solver script in a separate runtime, which it shells out to - so keeping yt-dlp itself current, which the app already does on every launch, does nothing for this. Today it costs formats; "deprecated" means it will eventually cost the download. Deno is the runtime yt-dlp enables by default, and it goes into the same `bin` folder as the bundled ffmpeg, which the sidecar already puts at the front of its PATH before any download runs - so there is no flag to pass and nothing else wired up. Non-fatal like the rest of the bundled tools: if the download fails, generating works exactly as it did before. Existing installs pick it up from the **Update AI tools** button; no reinstall needed. Reported by the user.

- **A library update broke every generation at the pitch step.** "Failed to generate the package: SwiftF0.__init__() got an unexpected keyword argument 'confidence_threshold'". swift-f0 was held only by a floor (`>=0.1.2`), so a routine run of the setup pulled 0.2.0 - which broke the API in THREE places at once, not the one the error names: the constructor stopped accepting the analysis parameters, `detect_from_array` became `detect`, and the `voicing` field disappeared altogether. `pipeline/pitch.py` is now on the 0.2.x API and decides voicing itself, comparing confidence against the same 0.85 threshold the old library applied internally. Checked rather than assumed, with both versions installed side by side on identical audio: on a voice-like signal (a harmonic stack with vibrato) 0.2.0 agrees with 0.1.2 on 124 of 125 frames at that threshold, with a median pitch difference of 1.3 Hz - so the tuning still holds. On a PURE SINE the two diverge badly, 0.2.0 being far less confident in an artificial tone, which is worth knowing but is not singing. The requirement now carries a ceiling of `<0.3`, so the next breaking change shows up as a refused install rather than a failed generation, and asks for the `[audio]` extra: the library imports `soxr` to resample our 44.1 kHz vocal down to the 16 kHz the model runs at, but only declares it as an extra - until now it had been arriving by luck, as a dependency of librosa. The same trap as the audioread one recorded earlier. Reported by the user.

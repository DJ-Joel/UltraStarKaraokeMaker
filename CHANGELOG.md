...
- **Um início de linha do .lrc podia deixar a primeira palavra do verso segundos à frente do resto dele.** O `seed_line_anchors` protegia só um lado: recusava início de linha que caísse DEPOIS da âncora medida seguinte, mas aceitava numa boa um que caísse muito ANTES. Caso real (2026-09-06, "Ministry - Effigy (I'm Not An)"): o `.lrc` abre "But them burning pictures of my head to toe, hey!" em 40,56s, enquanto o WhisperX MEDIU o resto dessa mesma linha a partir de 45,24s. A semente entrou em "But" e as duas fontes ficaram emendadas no mesmo verso — a primeira palavra 4,7s adiantada e sozinha, depois um buraco até o resto da frase. Pior: a linha anterior ficou espremida em 1,58s para sete palavras, entalada entre as duas sementes. Agora um início de linha é recusado quando fica mais de 3 segundos antes da primeira palavra MEDIDA da PRÓPRIA linha: dentro de um mesmo verso, uma medição tirada desta gravação ganha de um timestamp de arquivo. A palavra volta para a interpolação, que a espalha entre as vizinhas — sem buraco e sem verso espremido. Limitado de propósito a UMA linha, porque um vão grande ENTRE linhas é normal (intervalo instrumental) e julgar pela próxima âncora global recusaria sementes boas justamente aí. Pulado no modo override, junto das checagens de monotonicidade que já existiam, para o caminho de reconhecimento baixo que consertou o "Cause & Effect - Inside Out" não regredir.

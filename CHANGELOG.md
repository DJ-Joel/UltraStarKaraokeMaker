...
- **O setup passa a instalar o Deno, o runtime de JavaScript que o yt-dlp precisa para o YouTube.** Os downloads começaram a imprimir "No supported JavaScript runtime could be found ... YouTube extraction without a JS runtime has been deprecated, and some formats may be missing". O YouTube entrega desafios em JavaScript e o yt-dlp os resolve rodando um script solucionador num runtime SEPARADO, que ele chama por fora - então manter o próprio yt-dlp em dia, coisa que o app já faz a cada abertura, não resolve nada aqui. Hoje custa formatos; "deprecated" quer dizer que um dia vai custar o download inteiro. O Deno é o runtime que o yt-dlp habilita por padrão, e vai para a mesma pasta `bin` do ffmpeg embutido, que o sidecar já coloca na FRENTE do PATH antes de qualquer download - então não há flag para passar nem mais nada para ligar. Não-fatal como o resto das ferramentas embutidas: se o download falhar, gerar continua funcionando exatamente como antes. Quem já tem o app instalado pega isso pelo botão **Atualizar ferramentas de IA**; não precisa reinstalar. Relatado pelo usuário.

- **Uma atualização de biblioteca derrubou toda geração na etapa de pitch.** "Falha ao gerar o pacote: SwiftF0.__init__() got an unexpected keyword argument 'confidence_threshold'". O swift-f0 estava preso só por um piso (`>=0.1.2`), então uma passada rotineira do setup trouxe a 0.2.0 - que quebrou a API em TRÊS pontos de uma vez, e não no único que o erro mostra: o construtor deixou de aceitar os parâmetros de análise, o `detect_from_array` virou `detect`, e o campo `voicing` sumiu de vez. O `pipeline/pitch.py` agora está na API 0.2.x e decide sozinho o que é vozeado, comparando a confiança com o mesmo limiar de 0.85 que a versão antiga aplicava por dentro. Conferido em vez de suposto, com as duas versões instaladas lado a lado e o mesmo áudio: num sinal parecido com voz (série harmônica com vibrato) a 0.2.0 concorda com a 0.1.2 em 124 dos 125 quadros nesse limiar, com diferença mediana de 1,3 Hz no F0 - ou seja, a calibragem continua valendo. Num seno PURO as duas divergem muito, com a 0.2.0 bem menos confiante num tom artificial, o que é bom saber mas não é canto. A dependência passa a ter teto `<0.3`, pra próxima quebra aparecer como instalação recusada e não como geração falhada, e pede o extra `[audio]`: a lib IMPORTA o `soxr` pra reamostrar nosso vocal de 44,1 kHz para os 16 kHz em que o modelo roda, mas só o declara como extra - até agora ele vinha por sorte, de carona no librosa. Mesma armadilha do audioread já registrada aqui. Relatado pelo usuário.
